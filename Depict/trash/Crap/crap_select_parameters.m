...
elseif(isempty(The_mask)) 
  strerr=strcat('Please select mask before');
  herror1 = errordlg(strerr,'error1');
else

The_files_to_cluster

size(The_files_to_cluster)
...
fgSel = crap_figure('GetWin','SelectPar');
...
hvol_begin_val=uicontrol('Style','edit','string','1','Units','normalized','Position',[0.125 0.7 0.05 0.05]);
hvol_begin=uicontrol('Style', 'slider','Callback',@(hvol_begin,eventdata)slider_moved(hvol_begin,eventdata,hvol_begin_val), 'String','1','Units','normalized','Position',[0.05 0.75 0.2 0.05],'Min',1,'Max',nvols,'Value',1,'SliderStep',[1/(nvols-1),1]);
...
hvol_end_val=uicontrol('Style','edit','string','1','Units','normalized','Position',[0.475 0.7 0.05 0.05]);
hvol_end=uicontrol('Style', 'slider','Callback',@(hvol_end,eventdata)slider_moved(hvol_end,eventdata,hvol_end_val), 'String','1','Units','normalized','Position',[0.4 0.75 0.2 0.05],'Min',1,'Max',nvols,'Value',1,'SliderStep',[1/(nvols-1),1]);
...
hvol_len_val=uicontrol('Style','edit','string','1','Units','normalized','Position',[0.825 0.7 0.05 0.05]);
hvol_len=uicontrol('Style', 'slider','Callback',@(hvol_len,eventdata)slider_moved(hvol_len,eventdata,hvol_len_val), 'String','1','Units','normalized','Position',[0.75 0.75 0.2 0.05],'Min',1,'Max',nvols,'Value',1,'SliderStep',[1/(nvols-1),1]);
...
NCUT_val=uicontrol('Style','edit','string','200','Units','normalized','Position',[0.125 0.55 0.05 0.05]);

txt5 = uicontrol(fgSel,'Style','text','Units','normalized','Position',[0.40 0.6 0.2 0.05],'String','Neighbor filter');
SPATIALCUT_val=uicontrol('Style','edit','string','5','Units','normalized','Position',[0.475 0.55 0.05 0.05]);

txt6 = uicontrol(fgSel,'Style','text','Units','normalized','Position',[0.75 0.6 0.2 0.05],'String','Density filter');
RHO_val=uicontrol('Style','edit','string','0','Units','normalized','Position',[0.825 0.55 0.05 0.05]);

txt7 = uicontrol(fgSel,'Style','text','Units','normalized','Position',[0.05 0.4 0.2 0.05],'String','Maximum number of clusters');
NCLUST_MAX_val=uicontrol('Style','edit','string','20','Units','normalized','Position',[0.125 0.35 0.05 0.05]);


hinteractive = uicontrol(fgSel,'Style','checkbox','String','Interactive','Units','normalized','Position',[0.05,0.25,0.2,0.05],'Value',0);

hconnectedcut = uicontrol(fgSel,'Style','checkbox','String','Cut isolated regions','Units','normalized','Position',[0.05,0.15,0.3,0.05],'Value',0);
