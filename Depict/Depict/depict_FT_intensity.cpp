...
// mexEvalString("drawnow;"); // like cout.flush() 
 ...
 double *power=new double[omega]; //  power at each frequency
 double *pow_of_om=new double[omega];  // 
...
//************************************** CALCOLO FT NORMALIZZATA ********************************************// 
 ...
 for(int i1=0; i1<NC; i1++) {
    ...
    // output intensities given by inverse FT 
 ...
 if(only_power==true){
  ...
  delete ftr,fti,frr,fri,cos_ux,sin_ux,power,pow_of_om; 
