function varargout=depict_figure(varargin)
...
case 'createwin'
% F=depict_figure('CreateWin',Tag,Name,Visible)
...
if ~isempty(Name)
        set(F,'Name',sprintf('%s%s',[spm('ver') ' Depict Toolbox'],...
                spm('GetUser',' (%s)')),'NumberTitle','off')
end

%depict_figure('FigContextMenu',F);
...
if isempty(F)
        if ischar(Tag)
                switch Tag, case 'Interactive'
                        F = spm('CreateIntWin');
                otherwise
                        F = depict_figure('createwin',Tag,Tag);
                end
        end
else
